class CallMonitorTestCard extends HTMLElement {
  ...
  _filterCalls(calls, voicemails) {
    ...
    const hasMatchingVoicemail = (call) => {
      ...
      return voicemails.some((message) => {
        ...
        const messageDate = this._parseVoicemailDate(message.date);
  ...
  _voicemailTimestamp(message) {
    const date = this._parseVoicemailDate(message.date);
    return date && !Number.isNaN(date.getTime()) ? date.getTime() : 0;
  }

  _formatVoicemailDuration(message) {
    ...
    if (/^\d+$/.test(rawDuration)) {
      ...
      if (rawSeconds < 60) return `${rawSeconds} Sek.`;
  ...
  _renderCallRow(call, phonebooks) {
  ...
  _renderVoicemailRow(message, phonebooks) {
    const callerNumber = message.caller || "unterdrückte Rufnummer";
    const callerName = message.caller_name || "";
    const caller = callerName || callerNumber || "Unbekannter Anrufer";
    const formattedDuration = this._formatVoicemailDuration(message);
    const duration = formattedDuration
      ? ` · ${this._escape(formattedDuration)}`
      : "";
    const voicemailDateObject = this._parseVoicemailDate(message.date);
    const dateText =
    ...
    const loading = this._voicemailLoading === message.media_source_id;
      ...
      <div class="call-row voicemail-row">
        ...
        <div class="call-main">
          ...
          <div class="call-details">
            ${callerName ? `${this._escape(callerNumber)} · ` : ""}${dateText}${duration}
          </div>
        </div>
  ...
  _render() {
    ...
    const rawCallIds = new Set(calls.map((call) => call.call_id));
    for (const callId of this._optimisticallyDeletedCallIds) {
      if (!rawCallIds.has(callId)) {
        this._optimisticallyDeletedCallIds.delete(callId);
      }
    }
    ...
    const rows = visibleCalls
      .map((call) => this._renderCallRow(call, phonebooks))
      .join("");

    const callEntries = visibleCalls.map((call) => ({
      timestamp: call.timestamp ? new Date(call.timestamp).getTime() : 0,
      html: this._renderCallRow(call, phonebooks),
    }));

    const voicemailRows = voicemails
      .slice(0, maxCalls)
      .map((message) => this._renderVoicemailRow(message, phonebooks))
      .join("");

    const voicemailEntries = voicemails.map((message) => ({
      timestamp: this._voicemailTimestamp(message),
      html: this._renderVoicemailRow(message, phonebooks),
    }));
